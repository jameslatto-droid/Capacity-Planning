...
export function UtilisationHeatmap({ rows, months }: Props) {
  ...
  const minWidth = 144 + months.length * 58
        ...
        <div className="space-y-1">
          {rows.map((row, ri) => (
            ...
            >
              ...
              <div className="flex flex-1 rounded-lg overflow-hidden h-10" style={{ gap: '1px' }}>
                {row.values.map((v) => {
                  ...
                  return (
                    ...
                    >
                      {!isEmpty && (
                        <span
                          className="flex flex-col items-center justify-center gap-0.5 text-[10px] font-semibold tabular select-none leading-none"
                          style={{
                            color: labelColor(v.utilisation),
                            textShadow: v.utilisation > 1 ? '0 0 6px rgba(0,0,0,0.4)' : 'none',
                          }}
                        >
                          <span>{formatPercent(v.utilisation)}</span>
                          <span style={{ opacity: 0.78 }}>{Math.round(v.allocatedHours)}h</span>
                        </span>
                      )}
                    </div>
                  )
