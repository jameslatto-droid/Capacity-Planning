...
export function ProjectForm({ initial, resources, onSave, onCancel }: Props) {
  ...
  const [startMonth, setStartMonth] = useState(initial?.startMonth ?? '2025-01')
  const [endMonth, setEndMonth] = useState(initial?.endMonth ?? '2025-12')
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-3">
        ...
        <Select label="Status" value={status} onChange={(e) => setStatus(e.target.value as ProjectStatus)} options={[
      ...
      <div className="flex gap-3">
        <Select label="Priority" value={priority} onChange={(e) => setPriority(e.target.value as ProjectPriority)} options={[
          { value: 'critical', label: 'Critical' },
          { value: 'high', label: 'High' },
          { value: 'medium', label: 'Medium' },
          { value: 'low', label: 'Low' },
        ]} />
        <Select label="Flexibility" value={flexibility} onChange={(e) => setFlexibility(e.target.value as ProjectFlexibility)} options={[
      ...
      <div className="flex gap-3">
        <Input label="Start Month (YYYY-MM)" value={startMonth} onChange={(e) => setStartMonth(e.target.value)} type="month" />
        <Input label="End Month (YYYY-MM)" value={endMonth} onChange={(e) => setEndMonth(e.target.value)} type="month" />
      </div>
